...
public class ProductsFacade extends AbstractFacade<Products> implements ProductsFacadeLocal {
  ...
  public List<Products> getProductPagination(int currentPage, int recordsPerPage) {
    try {
      return em.createQuery("select e from Products e order by e.prodId DESC", Products.class).setFirstResult(currentPage * recordsPerPage - recordsPerPage).setMaxResults(recordsPerPage).getResultList();
    } catch (Exception ex) {
  ...
  public List<Products> searchWithPagination(String txtSearch, int currentPage, int recordsPerPage) {
    try {
      return em.createQuery("select e from Products e where e.name like :txtSearch order by e.prodId DESC", Products.class).setParameter("txtSearch", "%" + txtSearch + "%").setFirstResult(currentPage * recordsPerPage - recordsPerPage).setMaxResults(recordsPerPage).getResultList();
    } catch (Exception ex) {
      System.out.println(ex);
      return null;
    }
  ...
  public List<Products> searchCateWithPagination(Categories category, int currentPage, int recordsPerPage) {
    try {
      return em.createQuery("select e from Products e where e.cateId = :txtSearch order by e.prodId DESC", Products.class).setParameter("txtSearch", category).setFirstResult(currentPage * recordsPerPage - recordsPerPage).setMaxResults(recordsPerPage).getResultList();
    } catch (Exception ex) {
      System.out.println(ex);
      return null;
    }
